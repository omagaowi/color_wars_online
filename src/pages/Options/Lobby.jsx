...
import {
  ...
  colors,
  gameDataStore,
} from "../../utils/online/otherStores.jsx";
...
const Lobby = () => {
  ...
  handleUserEvents = (data) => {
    ...
    if (data.player.playerID == userData.playerID) {
      if (data.action == "joined") {
        setIsConnected(data.room.roomID);
      }
  ...
  handleStartEvent = (data) => {
    console.log(currentRoom, isConnected);
  ...
  const startGame = async (boxes) => {
    try {
      const data = {
        ...
        gameInfo: {
          playOrder: generateGrid(currentRoomPlayers.length, false, boxes)
            .order,
          playGrid: generateGrid(currentRoomPlayers.length, false, boxes).rows,
          playBoxes: generateGrid(currentRoomPlayers.length, false, boxes)
            .boxes,
        },
      };
      // console.log(data)
  ...
  return (
    <div className="options-container lobby">
      ...
      {loading ? (
        ...
        <>
          {error ? (
            ...
            <>
              <div
                className={`lobby-grid player${currentRoomPlayers.length}mode`}
              >
                  ...
                  <div className="player-name-plate">
                    ...
                    <h3 className="player-name">
                      {currentRoomPlayers[0] ? (
                        <>
                          {`${currentRoomPlayers[0].name} ${
                            currentRoomPlayers[0].playerID == userData.playerID
                              ? "(me)"
                              : ""
                          }`}
                        </>
                  ...
                  <div className="player-name-plate">
                    ...
                    <h3 className="player-name">
                      {currentRoomPlayers[1] ? (
                        <>
                          {`${currentRoomPlayers[1].name} ${
                            currentRoomPlayers[1].playerID == userData.playerID
                              ? "(me)"
                              : ""
                          }`}
                        </>
                  ...
                  <div className="player-name-plate">
                    ...
                    <h3 className="player-name">
                      {currentRoomPlayers[2] ? (
                        <>
                          {`${currentRoomPlayers[2].name} ${
                            currentRoomPlayers[2].playerID == userData.playerID
                              ? "(me)"
                              : ""
                          }`}
                        </>
                  ...
                  <div className="player-name-plate">
                    ...
                    <h3 className="player-name">
                      {currentRoomPlayers[3] ? (
                        <>
                          {`${currentRoomPlayers[3].name} ${
                            currentRoomPlayers[3].playerID == userData.playerID
                              ? "(me)"
                              : ""
                          }`}
                        </>
              ...
              <div className="lobby-actions">
                {isAdmin ? (
                  <>
                    <button
                      style={{
                        pointerEvents: startGameLoading ? "none" : "all",
                      }}
                      onClick={() => {
      ...
      <OnClickOutside currentState={ showShare } setState={ setShowShare }>
        <ShareModal />
      </OnClickOutside>
    </div>
  );
